...
export class Converter {
  ...
  constructor(strapi: Strapi, config: PluginConfig) {
    ...
    this.interfaceBuilder = InterfaceBuilderFactory.getInterfaceBuilder(
      strapi,
      this.commonHelpers,
      config
    );
    this.commonHelpers.printVerboseLog(
  ...
  public SchemasToTs(): void {
    ...
    const commonSchemas: SchemaInfo[] =
      this.interfaceBuilder.generateCommonSchemas(this.commonFolderModelsPath);
    const apiSchemas: SchemaInfo[] = this.getSchemas(
      strapi.dirs.app.api,
      SchemaSource.Api
    );
    const componentSchemas: SchemaInfo[] = this.getSchemas(
      strapi.dirs.app.components,
    ...
    for (const schema of schemas.filter(
      (x) => x.source !== SchemaSource.Common
    )) {
  ...
  */
  private adjustComponentsWhoseNamesWouldCollide(
    componentSchemas: SchemaInfo[]
  ) {
    for (const componentSchema of componentSchemas.filter(
      (x) => x.needsComponentSuffix
    )) {
      const component: SchemaInfo = componentSchemas.find(
        (x) =>
          x.pascalName === componentSchema.pascalName && !x.needsComponentSuffix
      );
      if (component) {
  ...
  private setCommonInterfacesFolder() {
    this.commonFolderModelsPath =
      FileHelpers.ensureFolderPathExistRecursive("");
  }

  private getSchemas(
    folderPath: string,
    schemaSource: SchemaSource,
    apiSchemas?: SchemaInfo[]
  ): SchemaInfo[] {
    const files: string[] = [];
  ...
  private parseSchema(
    ...
    switch (schemaSource) {
      ...
      case SchemaSource.Component:
        ...
        folder = path.join(
          path.dirname(file),
          this.componentInterfacesFolderName
        );
    ...
    let needsComponentSuffix: boolean =
      schemaSource === SchemaSource.Component &&
      (this.config.alwaysAddComponentSuffix ||
  ...
  private async writeInterfacesFile(schemas: SchemaInfo[]) {
    let interfacesFileContent =
      await this.interfaceBuilder.buildInterfacesFileContent(schemas);
    ...
    FileHelpers.writeInterfaceFile(
      "",
      "contentTypes.d.ts",

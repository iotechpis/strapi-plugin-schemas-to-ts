...
export abstract class InterfaceBuilder {
  ...
  public abstract addVersionSpecificCommonSchemas(
    commonSchemas: SchemaInfo[],
    commonFolderModelsPath: string
  ): SchemaInfo[];

  protected addCommonSchema(
    schemas: SchemaInfo[],
    commonFolderModelsPath: string,
    pascalName: string,
  ...
  private convertToInterface(schemaInfo: SchemaInfo, allSchemas: SchemaInfo[]) {
    ...
    const builtInterface: InterfaceBuilderResult = this.buildInterfaceText(
      schemaInfo,
      allSchemas
    );
  ...
  private buildInterfaceText(
    schemaInfo: SchemaInfo,
    allSchemas: SchemaInfo[]
  ): InterfaceBuilderResult {
    ...
    for (const attribute of attributes) {
      ...
      // -------------------------------------------------
        ...
        const componentInfo: SchemaInfo = this.getAttributeComponentInfo(
          propertyType,
          allSchemas
        );
      ...
      // -------------------------------------------------
        ...
        let enumName: string = CommonHelpers.capitalizeFirstLetter(
          pascalCase(originalPropertyName)
        );
        if (
          this.config.alwaysAddEnumSuffix ||
          originalPropertyName.toLowerCase() ===
            schemaInfo.pascalName.toLowerCase()
        ) {
          enumName += "Enum";
        }
   ...
   */
  private getAttributeComponentInfo(
    propertyType: string,
    allSchemas: SchemaInfo[]
  ): SchemaInfo {
    function isComponentWithoutSuffix(schemaInfo: SchemaInfo): unknown {
      return (
        !schemaInfo.needsComponentSuffix &&
        schemaInfo.pascalName === propertyType
      );
    }
    function isComponentWithSuffix(schemaInfo: SchemaInfo): unknown {
      return (
        schemaInfo.needsComponentSuffix &&
        schemaInfo.pascalName === `${propertyType}Component`
      );
    }
    ...
    return allSchemas.find(
      (schemaInfo) =>
        ...
        (isComponentWithoutSuffix(schemaInfo) ||
          isComponentWithSuffix(schemaInfo))
    );
  }
